package br.com.certisign.certisigner.pem;
...
public final class PEMUtilities {
    public static byte[] crypt(boolean encrypt, String provider, byte[] bytes,
    ...
    private static SecretKey getKey(char[] password, String algorithm,
            int keyLength, byte[] salt) throws IOException {
        return getKey(password, algorithm, keyLength, salt, false);
    }

    private static SecretKey getKey(char[] password, String algorithm,
            int keyLength, byte[] salt, boolean des2) throws IOException {
        OpenSSLPBEParametersGenerator pGen = new OpenSSLPBEParametersGenerator();

        pGen.init(PBEParametersGenerator.PKCS5PasswordToBytes(password),
                salt);

        KeyParameter keyParam;
        keyParam = (KeyParameter) pGen
                .generateDerivedParameters(keyLength * 8);
