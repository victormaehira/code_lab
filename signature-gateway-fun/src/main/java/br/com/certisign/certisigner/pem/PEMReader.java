        ...
        while ((line = readLine()) != null) {
            ...
            if (line.indexOf("-----BEGIN RSA PRIVATE KEY") != -1) {
                try {
                    ...
                    throw new IOException(
                            "problem creating RSA private key: "
                                    + e.toString());
                }
            }
            if (line.indexOf("-----BEGIN DSA PRIVATE KEY") != -1) {
                try {
                    ...
                    throw new IOException(
                            "problem creating DSA private key: "
                                    + e.toString());
                }
            }           
        }
    ...
    private PublicKey readPublicKey(String endMarker) throws IOException {
        ...
        for (int i = 0; i < algorithms.length; i++) {
            try {
                ...
                throw new RuntimeException("can't find provider "
                        + provider);
            }
        }
    ...
    private ContentInfo readPKCS7(String endMarker) throws IOException {
        ...
        if (buf.length() != 0) {
            throw new RuntimeException(
                    "base64 data appears to be truncated");
        }
        ...
        try {
            ...
            throw new IOException("problem parsing PKCS7 object: "
                    + e.toString());
        }
    }
